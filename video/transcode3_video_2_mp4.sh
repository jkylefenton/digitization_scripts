...
for index in 1 2 3 4; do
  ...
  for i in $( ls *.${suff[index]} ); do
    ...
    if [ -e `basename $i .${suff[index]}`.mp4 ]; then
      echo "MPEG4 for $i already exists"
    else
      echo "Transcoding $i"
      ffmpeg -i $i -an -pass 1 -vcodec libx264 -b 512k -r 29.97 -flags +loop -cmp +chroma -partitions 0 -me_method epzs -subq 1 -trellis 0 -refs 1 -coder 0 -me_range 16 -g 300 -keyint_min 25 -sc_threshold 40 -i_qfactor 0.71 -bt 512k -maxrate 768k -bufsize 2M -rc_eq 'blurCplx^(1-qComp)' -qcomp 0.6 -qmin 10 -qmax 51 -qdiff 4 -level 13 -deinterlace -y -f mp4 `basename $i .${suff[index]}`.mp4
